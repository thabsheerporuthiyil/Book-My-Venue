"""
Global DRF Exception Handler for the Auth Service.

This handler ensures that EVERY error response — whether it's a business
logic exception, a DRF validation error, or an unexpected server crash —
...
This prevents raw HTML tracebacks from leaking to API consumers
and gives frontend/mobile clients stable error codes to branch on.
"""
...
from apps.accounts.core.exceptions import AccountBaseException
...
def custom_exception_handler(exc, context):
    ...
    Priority order:
        1. Our custom AccountBaseException subclasses (business logic).
    ...
    # -------------------------------------------------------------------------
    if isinstance(exc, AccountBaseException):
        logger.warning(
    ...
    # -------------------------------------------------------------------------
    # 2. DRF Built-in Exceptions (let DRF handle first, then reformat)
    # -------------------------------------------------------------------------
    response = drf_exception_handler(exc, context)

    if response is not None:
        # DRF handled it — reformat into our standard envelope
